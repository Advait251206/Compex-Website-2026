...
function ParticleSphere(props: any) {
  ...
  const [positions, colors] = useMemo(() => {
    const count = 5000; // Reduced slightly for performance on all devices
...
function TechShape({ position, color, speed, scale }: TechShapeProps) {
...
// --- 5. Meteor Component ---
const Meteor = ({ texture }: { texture: THREE.Texture }) => {
  const mesh = useRef<THREE.Mesh>(null);
